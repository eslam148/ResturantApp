    ...
    public partial class Inventory : Form
    {
        ...
        private void comboBoxAddItem_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (Exist.Checked==true)
            {
                ...
                if (comboBoxAddItem.SelectedIndex!=-1)
                {
                    ...
                    QuntatyItem.Value = 1;
                    dataGridViewItem.Rows.Add(comboBoxAddItem.SelectedItem, item.SellPrice.ToString(), item.BuyPrice.ToString(), item.Quantity.ToString(),((CategoryData)comboCatagory.SelectedItem).Name, textBoxAddItemSupplier.Text);
        ...
        private void CusomerPayCompo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (billID.Count == 0)
            {
                ...
                BillIDPayCompo.DisplayMember ="s";
               BillIDPayCompo.DataSource = s;
            }
            else {
                BillIDPayCompo.DisplayMember = "billID";
